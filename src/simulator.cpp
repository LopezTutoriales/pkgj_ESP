...
void pkgi_delete_dir(const std::string& path)
{
    ...
    if (!dfd)
        throw formatEx<std::runtime_error>(
                "fallo al abrir({}): {}", path, strerror(errno));
    ...
    while ((dir = readdir(dfd)))
    {
        ...
        else
        {
            ...
            if (ret < 0)
                throw formatEx<std::runtime_error>(
                        "fallo unlink({}): {}", new_path, strerror(errno));
        }
    }
    ...
    if (res < 0)
        throw formatEx<std::runtime_error>(
                "fallo rmdir({}): {}", path, strerror(errno));
}
...
std::vector<uint8_t> pkgi_load(const std::string& path)
{
    ...
    if (fd < 0)
        throw std::runtime_error(fmt::format(
                "abrir({}) fallo: {:#08x}", path, static_cast<uint32_t>(fd)));
...
void* pkgi_create(const std::string& path)
{
    ...
    int fd = open(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
    if (fd < 0)
        throw std::runtime_error("pkgi_create fallo");
