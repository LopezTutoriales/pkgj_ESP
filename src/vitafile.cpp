...
void pkgi_rm(const char* file)
{
    ...
    if (err < 0)
    {
        LOG("error borrando archivo %s, err=0x%08x", file, err);
    }
}
...
int64_t pkgi_get_size(const char* path)
{
    ...
    if (err < 0)
    {
        LOG("imposible obtener tamaño de %s, err=0x%08x", path, err);
        return -1;
    }
    return stat.st_size;
}
...
{
    LOG("sceIoOpen openrw en %s", path);
    SceUID fd = sceIoOpen(path, SCE_O_RDWR, 0777);
    if (fd < 0)
    {
        LOG("fallo openrw %s, err=0x%08x", path, fd);
        return NULL;
    }
    LOG("sceIoOpen devolvio fd=%d", fd);
...
{
    LOG("sceIoOpen agrego en %s", path);
    SceUID fd =
            sceIoOpen(path, SCE_O_WRONLY | SCE_O_CREAT | SCE_O_APPEND, 0777);
    if (fd < 0)
    {
        LOG("imposible agregar %s, err=0x%08x", path, fd);
        return NULL;
    }
    LOG("sceIoOpen devolvio fd=%d", fd);
...
void pkgi_close(void* f)
{
    ...
    LOG("cerrando archivo %d", fd);
    int err = sceIoClose(fd);
    if (err < 0)
    {
        LOG("error al cerrar 0x%08x", err);
    }
}
